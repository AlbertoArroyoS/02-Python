...
display = Entry(mi_frame, textvariable=digito_display, font="Arial 15")


display.grid(row=1, column=1, columnspan=4, pady=10)

display.config(background="black", fg="green", justify="right", width=15)
...
def suma(num):
    ...
    if resultado.is_integer():
        digito_display.set(int(resultado))
        operacion ="suma"
    else:
        operacion ="suma"
...
def total():
    ...
    if(resultado+float(digito_display.get())).is_integer():
        digito_display.set(int(resultado+float(digito_display.get())))
        resultado=0
    else:

        digito_display.set(resultado+float(digito_display.get()))
...
botonrest=Button(mi_frame, text="-", width=4, command= lambda:resta(digito_display.get()))
...
botonsum=Button(mi_frame, text="+", width=4, command= lambda:suma(digito_display.get()))
